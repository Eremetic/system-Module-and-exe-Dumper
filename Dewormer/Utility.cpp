...
#define Lower(a) Wide_Char_To_Lower(a)
...
WCHAR* Proc_Comp(ULONG pPid)
{
	...
	do
	{
		...
		{
			result = (WCHAR*)malloc(sizeof(WCHAR) * wcslen(pe32.szExeFile));
...
WCHAR* Wide_Char_To_Lower(IN WCHAR* string)
{
	...
	{
		lower[i] = towlower(string[i]);
	}
	result = (WCHAR*)malloc(sizeof(WCHAR) * wcslen(lower));
