...
bool Ex_Cmp(LPWSTR input)
{
	...
	if (RSHasher(ex, ToWstring(StringObf("dll"))) || RSHasher(ex, ToWstring(StringObf("exe"))) || RSHasher(ex, ToWstring(StringObf("sys"))))  return TRUE;
...
wchar_t* Char_2_Wchar(char* input)
{
	...
	size_t tmp = 0;
	mbstowcs_s(&tmp, wc, cSize, input, cSize);
