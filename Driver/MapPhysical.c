...
VOID Unmap_Physical_From_Virtual(IN ULONG_PTR virtualAddress, OUT PULONG status)
{
#ifdef _DEBUG 
	...
	if (!NT_SUCCESS(unmap_Status = ZwUnmapViewOfSection(ZwCurrentProcess(), C_PTR(virtualAddress))))
	{
#ifdef _DEBUG 
...
VOID Read_Mapped_Data(PVOID pBuffer, IN ULONG_PTR virtualAddress, IN SIZE_T length, OUT PULONG status)
{
#ifdef _DEBUG 
	...
	pCopy.VirtualAddress = C_PTR(virtualAddress);
