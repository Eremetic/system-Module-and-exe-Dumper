...
static NTSTATUS CloseDispatch(_In_ PDEVICE_OBJECT DeviceObject, _Inout_ PIRP Irp)
{
	UNREFERENCED_PARAMETER(DeviceObject);
	Irp->IoStatus.Information = 0;
...
static NTSTATUS Unload(IN PDRIVER_OBJECT DriverObject)
{
	...
	ObReferenceObjectByHandle(hThread, THREAD_ALL_ACCESS, *PsThreadType, KernelMode, &object, NULL);
	ZwClose(hThread);
